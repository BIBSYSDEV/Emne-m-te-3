...
Feature: User edits subject headings
  ...
  Scenario: TEKORD-user searches for a UDC-classification to find which subject term is used
    Given that the user is logged in
    And that there is a TEKORD subject heading system with terms
    When the user searches for a UDC number
  ...
  Scenario: TEKORD-user viewing term clicks link to see previously classified items
    ...
    Then they are shown a list of results in Alma or Oria (preferably restricted to their institution)

  Scenario: TEKORD-user sees TEKORD and UDC in the Oria/Alma interface
    Given that a user has searched for a TEKORD term in Oria or Alma
    When they view a result
    Then they see the TEKORD term and UDC classification marked clearly in some view

  Scenario: TEKORD user uses DDC mappings on existing items to generate a suggested UDK/TEKORD combination
